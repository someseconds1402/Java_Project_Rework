...
public class Controller implements Initializable {
	...
	private Room room = new Room("D:\\Don't Click Here\\2020-2\\OOP\\Project\\Demo1\\src\\input\\input.txt");
	private boolean check = true; // bien kiem tra da tao room hay chua, neu chua se khong the chuyen pane
	private final int N = room.getN();
	private final int zoom = 300;
...
//			flatViewDraw();
	...
	AnchorPane flatView;
	@FXML
	AnchorPane sideView1;
	@FXML
	AnchorPane sideView2;
	@FXML
	AnchorPane sideView3;
	@FXML
	AnchorPane sideView4;
	...
	public void flatViewDraw() {
		...
		double primaryX = room.getWidthRoom() / N;
		double primaryY = room.getLengthRoom() / N;
		
		Rectangle rect = new Rectangle(0, 0, room.getWidthRoom() *zoom, room.getLengthRoom() *zoom);
		rect.setFill(Color.YELLOW);
		group.getChildren().add(rect);
		
		ArrayList<Double> arr = room.couldnotSeePoint();
		for(int i = 0; i < arr.size(); i += 2) {
			Rectangle rectangle = new Rectangle(arr.get(i) *zoom, arr.get(i+1) *zoom, primaryX *zoom, primaryY *zoom);
	...
	public void sideView1Draw() {
		...
		Rectangle rect = new Rectangle(0, 0, room.getLengthRoom() *zoom, room.getHeightRoom() *zoom);
		rect.setFill(Color.YELLOW);
		group.getChildren().add(rect);
		
		ArrayList<Double> arr = room.couldnotSeePoint2(1, 1);
		for(int i = 0; i < arr.size(); i += 2) {
			Rectangle rectangle = new Rectangle(arr.get(i) *zoom, arr.get(i+1) *zoom, primaryY *zoom, primaryZ *zoom);
			group.getChildren().add(rectangle);
		}

		sideView1.getChildren().clear();
		sideView1.getChildren().add(group);
	}

	public void sideView2Draw() {
		...
		ArrayList<Double> arr = room.couldnotSeePoint2(1, 2);
		for(int i = 0; i < arr.size(); i += 2) {
			Rectangle rectangle = new Rectangle(arr.get(i) *zoom, arr.get(i+1) *zoom, primaryY *zoom, primaryZ *zoom);
			group.getChildren().add(rectangle);
		}

		sideView2.getChildren().clear();
		sideView2.getChildren().add(group);
	}
	
	public void sideView3Draw() {
		...
		sideView3.getChildren().clear();
		sideView3.getChildren().add(group);
	}
	
	public void sideView4Draw() {
		...
		ArrayList<Double> arr = room.couldnotSeePoint2(2, 2);
		for(int i = 0; i < arr.size(); i += 2) {
			Rectangle rectangle = new Rectangle(arr.get(i) *zoom, arr.get(i+1) *zoom, primaryY *zoom, primaryZ *zoom);
			group.getChildren().add(rectangle);
		}

		sideView4.getChildren().clear();
		sideView4.getChildren().add(group);
	}
	...
	// Show volume of the visible area of the camera 

	@FXML TextArea volumeTextArea;
	
	public void detailVolume() {
		volumeTextArea.setText(null);
		volumeTextArea.appendText("Volume of the visible area of the camera : \n" + room.couldSeeVolume());
	}
	
	
	// Export room's information
	
	public void exportInfo() {
		boolean checkExport = room.exportFile("D:\\Don't Click Here\\2020-2\\OOP\\Project\\Demo1\\src\\input\\export.txt");
		if(checkExport) {
			Dialog<String> dialog = new Dialog<>();
			dialog.setTitle("NOTIFICATION");
			dialog.setContentText("Export to file export.txt success!");
			dialog.getDialogPane().getButtonTypes().add(ButtonType.OK);
			dialog.show();
		} else {
			Dialog<String> dialog = new Dialog<>();
			dialog.setTitle("ERROR");
			dialog.setContentText("Can't open file!");
			dialog.getDialogPane().getButtonTypes().add(ButtonType.OK);
			dialog.show();
		}
	}
